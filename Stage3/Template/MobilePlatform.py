...
from InfraredSensors import TrackSensor

GPIO.setmode(GPIO.BCM)

#Class MobilePlatform to create a tank Object
class MobilePlatform:
    def __init__(self,H_Bridge,L_Motor,R_Motor,Dis_Sensor,L_TrackSensor,R_TrackSensor):
        ...
        self.__LeftTrackSensor = L_TrackSensor
        
        #Right Tracking Sensor
        self.__RightTrackSensor = R_TrackSensor
        ...
        #Variable to enable of disable tracking sensors
        self.__MonitorLanes = False
    ...
    def Drive(self):
        ...
        #Set the variable MonitorLanes to true allowing the Track to be detected
        self.__MonitorLanes = True
        
        """write a single line of code to instantiate a Thread Object with the reference name self.__LaneThread
            executing the LaneMonitor Method on a separate core of the CPU. Initially the thread is not started"""
        self.__LaneThread = threading.Thread(target = self.LaneMonitor)
        
        #Write a single line of code to start the thread to monitor the lanes
        self.__LaneThread.start()
        
    #Thread Method to monitor the lanes
    def LaneMonitor(self):
        while(self.__MonitorLanes):
            """Write an if condition to check whether the left lane is crossed by calling the DetectTrack method on the
                object self.__LeftTrackSensor. If True, print the message "Crossing Left lane" and make turn to get back in lane.
                If not True print the message "In lane"""
            if(not self.__LeftTrackSensor.DetectTrack()):
                print("Crossing Left lane")
                self.IRTurnLeft(35.0)
            else:
                print("In lane")

            """Write an if condition to check whether the right lane is crossed by calling the DetectTrack method on the
                object self.__RightTrackSensor. If True, print the message "Crossing Right lane" and make turn to get back in lane.
                If not True print the message "In lane"""
            if(not self.__RightTrackSensor.DetectTrack()):
                print("Crossing Right lane")
                self.IRTurnRight(35.0)
            else:
                print("In lane")
            time.sleep(.1)
    ...
    def Stop(self):
        ...
        """Check whether the __MonitorLanes is set to True indicating
            the lane monitoring thread was enabled. If true, set the __MonitorLanes
            to False and call the join method on the __LaneThread object"""
        if self.__MonitorLanes:
            self.__MonitorLanes = False
            self.__LaneThread.join()
    ...
    def IRTurnLeft(self,speed):
        ...
        self.__LeftMotor.Direction = Motor.BACKWARD  #Change the left motors direction to backward
        self.__RightMotor.Direction = Motor.FORWARD  #Change the right motors direction to forward
        self.__LeftMotor.Speed = speed                         #Set the speed of the left motor to the desired speed
        self.__RightMotor.Speed = speed                       #Set the speed of the right motor to the desired speed
        self.__LeftMotor.Run()                                         #Run the left motor with the new parameters
        self.__RightMotor.Run()                                       #Run the right motor with the new parameters
        time.sleep(.1)                                                      #Sleep so that the tank will turn
        self.__LeftMotor.Direction = LeftDir                    #Reset the left motors direction to its previous value
        self.__RightMotor.Direction = RightDir                #Reset the right motors direction to its previous value
        self.__LeftMotor.Speed = LeftSpeed                  #Reset the left motors speed to its previous value
        self.__RightMotor.Speed = RightSpeed              #Reset the right motors speed to its previous value
    ...
    def IRTurnRight(self,speed):
        ...
        self.__RightMotor.Direction = Motor.BACKWARD#Change the right motors direction to forward
        self.__LeftMotor.Speed = speed                         #Set the speed of the left motor to the desired speed
        self.__RightMotor.Speed = speed                       #Set the speed of the right motor to to the desired speed
        self.__LeftMotor.Run()                                         #Run the left motor with the new parameters
        self.__RightMotor.Run()                                       #Run the right motor with the new parameters
        time.sleep(.1)                                                       #Sleep so that the tank will turn
        self.__LeftMotor.Direction = LeftDir                    #Reset the left motors direction to its previous value
...
if __name__ == '__main__':
    ...
    LeftIRSensor_Pin = 17 #Requires PULL DOWN
    RightIRSensor_Pin = 27 #Requires PULL DOWN
    ...
    """Write a single line of code to instantiate an Infrared Sensor object of type TrackSensor with the
        reference name LLaneSensor using the following parameter:
        LeftIRSensor_Pin"""
    LLaneSensor = TrackSensor(LeftIRSensor_Pin)  #creating a onject of type TrackSensor with the parameters set above (LeftIRSensor_Pin) 
    
    """Write a single line of code to instantiate an Infrared Sensor object of type TrackSensor with the
        reference name RLaneSensor using the following parameter:
        RightIRSensor_Pin"""
    RLaneSensor = TrackSensor(RightIRSensor_Pin)    #creats a abject of type TrackSensor with the parameters set above (RightIRSensor_Pin)
    ...
    """Write a single line of code to instantiate a Mobile Platform Object with the reference name Tank
        ...
        Left TrackSensor object
        Right TrackSensor Object""" 
    Tank = MobilePlatform(hBridge,RightMotor,LeftMotor,DistanceSensor,LLaneSensor,RLaneSensor)     #Creating objects using the parameters. 
